...
- **GitHub Actions' built-in `schedule:` trigger is not exact-minute — and at 3-5 minute frequency it's not close.** This was originally accepted as a small trade-off, but real production runs (project history) showed gaps of 20–90+ minutes instead of the configured 3/5 minutes — GitHub documents `schedule:` as best-effort and throttles/delays it under load, especially at this frequency. That's not just cosmetic: `latest_signal()` only checks the single most recent closed candle (§3's stateless design), so a flip that happens mid-gap and isn't the trend anymore by the next delayed run is silently missed — no error, no email. The fix that was adopted: drop the `schedule:` trigger entirely (`eurusd_check.yml`/`xauusd_check.yml` now carry only `workflow_dispatch: {}`) and drive the real timing from an external precise pinger (cron-job.org, free tier) calling GitHub's workflow-dispatch API every exact 5/3 minutes — dispatch-triggered runs aren't subject to the same throttling as `schedule:` events. `session.py`'s window/day gate still runs on every single triggered run regardless of who triggered it, so an off-hours ping is still a harmless no-op. If cron-job.org is ever removed, the `schedule:` block would need to come back (accepting its known imprecision) or be replaced by an equivalent external trigger — don't leave the workflows with no scheduler at all. **`daily_journal.yml` never got this fix and still relies solely on `schedule:`** (`cron: "30 16 * * *"`, 22:00 IST) — caught in production (project history) when the journal simply didn't arrive for a day: the scheduled run hadn't fired over an hour after its cron time, matching the exact throttling behavior documented above. Unlike the live checks, the journal only runs once a day, so a `workflow_dispatch: {}` trigger exists as a manual fallback but nothing currently pings it externally — if this recurs, give `daily_journal.yml` the same cron-job.org treatment as the live checks.
- **The external pinger firing more often than configured causes duplicate alert emails, not just wasted runs.** Caught in production (project history): GitHub Actions run history showed both `eurusd_check.yml` and `xauusd_check.yml` being triggered roughly every 60 seconds — far more than the intended 5-minute/3-minute cadence — almost certainly duplicate or overlapping jobs on the cron-job.org side. Because `latest_signal()` only asks "is there a flip on the current latest closed candle" with no memory of prior runs, every extra trigger during the same still-current candle window re-detected the identical signal and would re-send the identical email. The fix: `src/alert_state.py` persists a small per-symbol "last alerted signal time" marker (`state/last_alert_eurusd.json` / `state/last_alert_xauusd.json`, committed back by the workflow the same way `journal_log.py` persists journal summaries), and `runner.py` checks it before sending — a signal is never re-emailed once alerted, no matter how many extra times the workflow gets triggered. This is a guard against duplicate *sends*, not a fix for over-triggering itself — excess triggers still burn Actions minutes and data-provider calls, so if this recurs, check the cron-job.org dashboard for duplicate/overlapping jobs on the same workflow.
...
/src
  ...
  alert_state.py                # persisted last-alerted-signal marker per symbol (state/last_alert_*.json) -- dedup guard against duplicate/overlapping external triggers, see §7
...
/tests
  ...
  test_runner.py                 # session/no-signal/signal/error-propagation/dedup control flow
  test_alert_state.py             # last-alerted-signal marker read/write/overwrite
...
## 14. Asia Sweep Reversals — the active alert system (added after the journal)

**This replaces HalfTrend as the live system.** HalfTrend's code, tests, workflows and journal are all kept and untouched, but **dormant** — the user disabled its cron-job.org pingers, so `eurusd_check.yml` / `xauusd_check.yml` / `daily_journal.yml` (all `workflow_dispatch`-only) simply never fire. **Do not delete HalfTrend code.** Everything in §1–§13 still describes how HalfTrend works; this section is the new system layered alongside it.

Source of truth: `reference/asia_sweep_source.pine` ("Asia Sweep Reversals [LuxAlgo]", MPL-2.0). Same rule as §3 — **for the strategy math the `.pine` file always wins**, and the port (`src/asia_sweep.py`) must be validated bar-for-bar against the live TradingView chart before it's trusted (`python -m src.validate_asia_sweep`). Logic drift from the Pine original is still the #1 risk in this codebase (§7).

### What it is

Watches **XAUUSD, EURUSD, GBPUSD, AUDUSD on 1-minute candles**. Detects the Asia session range (19:00–04:00 New York, DST-aware), a sweep of that range's high/low, then a CHoCH reversal entry off internal market structure. Sends **two Telegram notifications** (no email — see below):

1. **Asia High/Low Sweep** — the moment price wicks the completed Asia high or low. A heads-up: swept level + Asia high/mid/low + session date. No sizing (there is no trade yet).
2. **Entry Signal (LONG/SHORT)** — the CHoCH entry: full trade plan — Entry, Stop Loss (the internal swing ± buffer), **3 targets at 1R / 2R / 3R**, R:R per target, an ATR volatility line, and the per-account position-size + dollar-risk table (monospace block).

Plus a **journal** (daily / weekly / monthly / yearly) — one combined Telegram digest per period covering all 4 pairs, reusing the HalfTrend journal's outcome simulator and two-level compounding (§12) against its own persisted log `journal/asia_sweep_daily_log.json`.

### Decisions and why (all confirmed with the user)

- **Telegram-only, full content, own bot.** No email path at all. Every message carries the complete plan, not a "check your email" nudge. `ASIA_SWEEP_TELEGRAM_BOT_TOKEN` / `ASIA_SWEEP_TELEGRAM_CHAT_IDS` — a *separate* bot from the HalfTrend `TELEGRAM_*` one. Either unset → the run logs loudly to stderr and sends nothing; it never crashes (§2 fail-loud, minus turning a config gap into a crash).
- **1-minute, all 4 pairs.**
- **Data: OANDA only** (`EUR_USD` / `GBP_USD` / `AUD_USD` / `XAU_USD` all have native M1) — no Twelve Data, no new provider. yfinance stays the automatic fallback exactly as for HalfTrend. **yfinance's 1m FX data is visibly coarse/stale/forward-filled** (prices quantised to ~1.4-pip steps, closes flat for 10+ bars) — good enough for a fallback but the strategy really wants OANDA's true 1m. If a pair keeps showing the yfinance source label, check OANDA (`python -m src.health_check`).
- **Alert-active window: NY 04:00–12:00, Mon–Fri**, evaluated in `America/New_York` (so it doesn't drift across US daylight saving), `src/asia_sweep_session.py` — deliberately its own module, `src/session.py` (IST, HalfTrend) is left alone. Off-hours pings are harmless no-ops.
- **Trend filter defaults OFF.** Pine's own "Enable trend filter" default is `true`, but a 200-EMA "only short below / long above" filter on 1m suppresses this counter-trend Asia-sweep setup *almost entirely* — a bearish CHoCH right after a bullish Asia-high breakout is nearly always still above the EMA. Verified against the user's screenshot: the EURUSD SHORT they showed (~09:34 NY, 2026-08-27, entry ≈ 1.16566 / SL ≈ 1.16600) only forms with the filter off, and the port reproduces it exactly. Set `ASIA_SWEEP_TREND_FILTER=true` to restore the Pine default.
- **3-target ladder overrides the Pine single TP.** The script's take-profit is one 2R target (`rewardRiskInput`). The alert instead reports T1/T2/T3 = entry ± {1,2,3} × R (R = entry→stop distance), matching the HalfTrend emails. `reward_rr` is kept only to drive the **active-trade gating** — the ported one-trade-at-a-time logic frees a new entry on exactly the bar TradingView's marker would, i.e. when the 2R level (or the stop) is hit — so the port's entry cadence matches the chart even though the ladder shown is 1/2/3R.
- **Both crons via cron-job.org, not GitHub `schedule:`** — the 1-minute live check *and* the daily journal fire from cron-job.org calling each workflow's `workflow_dispatch` API. GitHub's `schedule:` throttles even at once-daily cadence (§7).
- **Journal: one combined digest across all 4 pairs**, not one message per pair. Daily/weekly R is simple-summed; the combined "Return by Account" for monthly/yearly compounds each pair's already-compounded period return together (all 4 pairs in one account).

### Ported-quirk notes (do not "fix" these)

- The SHORT entry gates on `internal_low` being **inside** the Asia range while the stop is placed at `internal_high`; LONG mirrors it (gates on `internal_high` in range, stop at `internal_low`). Script as written.
- `max_bars_after_sweep` (default 10) is counted from the **breakout** bar (`asia_high_break_bar` / `asia_low_break_bar`), not the sweep — they normally coincide.
- Only the **first** breakout per session per side arms the setup, and the setup disarms permanently once it fires or the next Asia session begins. Combined with the 10-bar window this makes entries genuinely infrequent (~1–3 per pair per week in practice) — that is the strategy, not a bug.
- `internal_trend` (0 / 1 / -1) is genuine bar-to-bar loop state.
- `ta.pivothigh(high, L, L)` is ported as a strict unique local extreme over the `2L+1`-bar window centred `L` bars back (ties → no pivot). If the TradingView cross-check shows a systematic internal-structure mismatch, this is the first knob to revisit (a `>=`-on-one-side variant produced ~3× more pivots but identical entry output on the data checked).

### Config

All `ASIA_SWEEP_*` env vars (global, or per-symbol with a `{SYMBOL}_` prefix) — see `.env.example` for the full list with defaults. The `ACCOUNT_SIZES` / `RISK_PERCENTAGES` matrix is shared with HalfTrend. GBPUSD/AUDUSD need **no** new sizing config — they are USD-quoted majors with identical pip math to EURUSD (`position_sizing.PIP_SYMBOLS`).

**`ASIA_SWEEP_MAX_BARS_AFTER_SWEEP=20` in this deployment, not the Pine default of 10.** Caught via a real missed-signal report (project history): an AUDUSD LONG on 25 Aug 2026 (Asia-low break 04:04 NY, CHoCH confirmed 04:22 NY — 18 bars later) was on the user's live chart but the port didn't fire it, because 18 bars exceeds the Pine default `maxBarsAfterSweepInput` of 10. The user confirmed their chart's actual setting is 20 bars (timezone was independently confirmed as `America/New_York`, ruling that out as a factor). Bar-by-bar replay against real OANDA 1m data confirmed the entry fires correctly once `ASIA_SWEEP_MAX_BARS_AFTER_SWEEP=20` is set — every other gate (session range, internal-high-in-range, close-in-range) already matched. If entries still look sparse/late vs. the live chart, re-check this value first before suspecting the port logic itself.

### Modules

`src/asia_sweep.py` (engine + `asia_sweep_params`), `asia_sweep_session.py` (NY window gate), `asia_sweep_telegram.py` (sweep + entry messages), `asia_sweep_runner.py` + `asia_sweep_check_runner.py` (live, all 4 pairs, per-pair isolation), `asia_sweep_journal.py` (outcome walker, reuses `trade_journal.simulate_trade_outcome`), `asia_sweep_journal_telegram.py` (combined digest), `asia_sweep_journal_runner.py` + `asia_sweep_journal_check_runner.py` (journal job), `validate_asia_sweep.py` (mandatory bar-for-bar validation). Data layer: `market_data_client.SYMBOL_INFO` / `TIMEFRAME_FETCH` + a `timeframe=` override threaded through `oanda_client` / `data_provider` (all backward-compatible — HalfTrend callers pass nothing and behave identically). `telegram_alert.send_telegram_notification` gained optional `token=` / `chat_ids=` / `parse_mode=`. `trade_journal`'s period reports gained an optional `path=`. Workflows: `.github/workflows/asia_sweep_check.yml`, `asia_sweep_journal.yml` (both `workflow_dispatch`-only). Tests: `test_asia_sweep*.py` (engine / session / telegram / journal / runner).

### Deploy checklist

1. GitHub repo secrets: `ASIA_SWEEP_TELEGRAM_BOT_TOKEN`, `ASIA_SWEEP_TELEGRAM_CHAT_IDS`, plus any `ASIA_SWEEP_*` overrides. (`OANDA_API_KEY`, `ACCOUNT_SIZES`, `RISK_PERCENTAGES` already exist.)
2. cron-job.org: one job every 1 minute → `POST .../actions/workflows/asia_sweep_check.yml/dispatches` `{"ref":"main"}`; one job once/day (after 12:00 NY) → `asia_sweep_journal.yml/dispatches`.
3. Disable the old HalfTrend cron-job.org jobs (EURUSD/XAUUSD checks + journal) — that is what makes HalfTrend dormant.
4. Validate against TradingView for several days across all 4 pairs before trusting it (§9, PRD Milestone 6).
